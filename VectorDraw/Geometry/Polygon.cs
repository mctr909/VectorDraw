...
namespace Geometry {
	public class Polygon {
		...
		static bool IsSelected(List<Surface> poly, Point cursor) {
			foreach (var po in poly) {
				if (Util.HasInnerPoint(po.a, po.o, po.b, cursor)) {
					return true;
				}
			}
			return false;
		...
		static List<Surface> GetSurface(PointF[] poly, EOrder order) {
			...
			do { // 最も遠くにある頂点(vo)の取得ループ
				...
				while (true) { // 頂点(vo)の移動ループ
					...
					var ob_x = vb.X - vo.X;
					var ob_y = vb.Y - vo.Y;
					aob_normal = (oa_x * ob_y - oa_y * ob_x) * (int)order;
					if (aob_normal < 0) {
						/*** 裏の場合 ***/
						reverse_count++;
					...
					for (uint i = 0; i < INDEX_COUNT; i++) {
						...
						var p = poly[i];
						if (Util.HasInnerPoint(va, vo, vb, p)) {
